...
public class GLES30Renderer implements GLSurfaceView.Renderer {
    ...
    final static int TEXTURE_ID_COW = 2;
    ...
    public void onSurfaceCreated(GL10 gl, javax.microedition.khronos.egl.EGLConfig config) {
        ...
        //mShadingProgram.set_up_scene_lights(mViewMatrix); // VIewMatrix 초기화가 안 되어 있네
        mShadingProgram.set_up_scene_lights(mCamera.GetViewMatrix());
        ...
        mBuilding.prepare();
        mBuilding.setTexture(AssetReader.getBitmapFromFile("me.jpg", mContext), TEXTURE_ID_BUILDING);

        nBytesPerVertex = 6*4; //!!!
        nBytesPerTriangles = nBytesPerVertex * 3; //!!!
        mCow = new Cow();
        mCow.addGeometry(AssetReader.readGeometry("cow_vn.geom", nBytesPerTriangles, mContext));
        mCow.prepare();
    }
    ...
    public void onDrawFrame(GL10 gl){ // 그리기 함수 ( = display )
        ...
        //mShadingProgram.set_lights0();
        //mShadingProgram.set_lights1();
        //mShadingProgram.set_lights2();
        ...
        if(MainActivity.Building == 1){
            if(MainActivity.Texture == 1)
                mShadingProgram.textureOn();
            else
                mShadingProgram.textureOff();
        ...
        Matrix.setIdentityM(mModelMatrix, 0);
        Matrix.translateM(mModelMatrix, 0, 100f, 100f, 40f);
        Matrix.scaleM(mModelMatrix, 0, 20f, 20f, 20f);
        ...
        mShadingProgram.setUpMaterialCow();
        mCow.draw();
        ...
        Matrix.translateM(mModelMatrix, 0, 115.0f, 65.0f, 60.0f);
    ...
    public void setLight0(){
        mShadingProgram.light[0].light_on = 1 - mShadingProgram.light[0].light_on;
    }
